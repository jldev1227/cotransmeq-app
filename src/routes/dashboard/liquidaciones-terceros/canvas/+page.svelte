<script lang="ts">
	...
	import { icoBorradores, icoCorreo, icoExcel, icoVer, icoZip } from '$lib/components/univer/iconos-canvas.svelte';
	 ...
	 *
	 * El modal compone el PDF de cada hoja con el detalle EN MEMORIA (el
	 * mismo `DocumentoHoja` del preview) y lo encola en el servidor, que lo
	 * renderiza con Chromium, lo adjunta y lo envía a nombre de contabilidad.
	 * El estado ENVIADO/cuándo vive en `liquidacion_tercero_envio`.
	 */
	let modalEnvios = $state(false);
		...
		try {
			...
			const hoja = previa
				? {
						...
						// El reparto por propietarios también sale del detalle: sin esto,
						// al guardar copropietarios la hoja y el documento seguían en
						// modo single (con las filas de impuesto ya multiplicadas por N)
						// hasta recargar la página entera.
						es_multi_propietario: completo.hoja.es_multi_propietario,
						copropietarios: completo.hoja.copropietarios
					}
				: completo.hoja;
...
<svelte:head>
	<title>Cierres finales {periodDisplay} (canvas) · Transmeralda</title>
</svelte:head>

<UniverToolbar
	title="CIERRES FINALES DE TERCEROS: {periodDisplay}"
	hoja={cierreActivoObj?.placa ?? ''}
...
{#snippet icoPropietarios()}
	<svg
		...
		stroke-linejoin="round"
	>
		<path d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493" />
		<path d="M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07" />
		<path d="M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0z" />
		<path d="M19.5 6.375a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
	</svg>
{/snippet}
...
<div class="cf-body">
	...
	<UniverSideRail
		...
		items={[
			...
			{
				id: 'propietarios',
				label: 'Propietarios de la placa',
				hint: 'Repartir el valor a pagar entre varios propietarios en cascada: el primero toma su % del total y los demás se reparten el remanente. Por defecto la hoja tiene uno solo.',
				icon: icoPropietarios,
				badge:
					(detalleActivo?.copropietarios?.length ?? 0) > 1
						? detalleActivo!.copropietarios.length
						: 0,
				disabled: !hojaEditable || !detalleActivo || !!accionEnCurso,
				disabledHint: motivoBloqueo,
				onSelect: () => (modalPropietarios = true)
			},
			{
				id: 'concepto',
			...
			},
			{
				...
				hint: `Enviar a cada tercero el PDF de su liquidación de ${periodDisplay}, con mensaje personalizado y adjuntos adicionales. Sale a nombre de contabilidad y deja constancia de a quién y cuándo se envió.`,
				icon: icoCorreo,
				disabled: indice.length === 0,
				disabledHint: 'El periodo no tiene hojas.',
				onSelect: () => (modalEnvios = true)
			},
			{
				id: 'borradores',
...
{#if modalPropietarios && cierreActivoObj && detalleActivo}
	<PropietariosCierreModal
		cierreId={cierreActivoObj.id}
		placa={cierreActivoObj.placa}
		periodo={periodDisplay}
		terceroTitular={{
			id: cierreActivoObj.tercero_id ?? null,
			nombre: cierreActivoObj.tercero_nombre || '',
			identificacion: null
		}}
		onClose={() => (modalPropietarios = false)}
		onGuardado={async () => {
			modalPropietarios = false;
			// Cambia la geometría de la hoja (aparece o desaparece la sección de
			// copropietarios y las filas de impuesto se multiplican por N), así
			// que se remonta el libro, igual que con los conductores.
			await conOverlay(
				{
					titulo: 'Aplicando reparto de propietarios',
					detalle: `${cierreActivoObj.placa} · rehaciendo la hoja con los impuestos repartidos en cascada.`
				},
				() => recargarCierre(cierreActivoObj.id)
			);
			toast.success(`Propietarios de ${cierreActivoObj.placa} actualizados`, {
				description: 'Impuestos repartidos por propietario y totales recalculados.'
			});
		}}
	/>
{/if}
