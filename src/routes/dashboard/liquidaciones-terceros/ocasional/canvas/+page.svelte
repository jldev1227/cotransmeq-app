<script lang="ts">
	...
	import ModalFilasOcasional from '$lib/components/univer/ModalFilasOcasional.svelte';
	...
	import ItemsDisponiblesOcasionalModal from '$lib/components/liquidaciones-terceros/ItemsDisponiblesOcasionalModal.svelte';
	...
	function conConceptosBase(conceptos: any[], mes: number, cfg: typeof configDescuentos) {
		...
		return ensureGastosVehiculo(
			ensureCostosLaborales(sinGastosRetiradosVacios(conceptos), cfg, alcance),
			alcance
		);
	}

	/**
	 * Los tres gastos que la hoja SEMBRABA y ya no: DOTACION, EXAMEN MÉDICO y
	 * COMBUSTIBLE (ver `GASTOS_VEHICULO`).
	 *
	 * No basta con dejar de sembrarlos: los meses que ya se guardaron los llevan
	 * dentro, así que seguirían apareciendo —en cero— hasta el fin de los
	 * tiempos. Se descartan AL LEER y, como el guardado reemplaza los conceptos
	 * en bloque, el mes se cura solo en el siguiente autoguardado. Mismo patrón
	 * que `TIPOS_RETIRADOS` en el canvas de ingresos.
	 *
	 * SOLO LOS QUE ESTÁN EN CERO. Un DOTACION con importe es un gasto real que
	 * alguien liquidó, y borrarlo cambiaría el total de un documento entregado.
	 * Ese se conserva y el builder lo pinta como fila añadida, con su nombre
	 * editable y su botón de quitar.
	 */
	const GASTOS_RETIRADOS = new Set(['DOTACION', 'EXAMEN_MEDICO', 'COMBUSTIBLE']);
	function sinGastosRetiradosVacios(conceptos: any[]): any[] {
		return conceptos.filter((c) => {
			if (c?.tipo !== 'GASTO_OPERATIVO') return true;
			if (!GASTOS_RETIRADOS.has(String(c.concepto))) return true;
			return (Number(c.valor_total) || 0) !== 0 || (Number(c.valor_unitario) || 0) !== 0;
		});
	}
	...
	/// Anticipos e impuestos del mes activo. Los anticipos son TODOS añadidos a
	/// mano —no hay filas fijas—; los impuestos se listan solo para consultarlos
	/// (ver el docblock de `ModalFilasOcasional`).
	let anticiposMes = $derived(
		(datosMesActivo?.conceptos ?? []).filter((c) => c.tipo === 'ANTICIPO')
	);
	let impuestosMes = $derived(
		(datosMesActivo?.conceptos ?? []).filter((c) => c.tipo === 'IMPUESTO')
	);
	...
	async function agregarFila(nuevo: {
		...
		try {
			...
			// entre recargas.
			// `orden` se cuenta DENTRO DE SU TIPO: las dos tablas se ordenan por
			// separado, y compartir el contador dejaría el primer anticipo con el
			// orden del último gasto.
			const orden =
				datos.conceptos
					.filter((c) => c.tipo === nuevo.tipo)
	...
	/**
	 * Envío por correo del MES ACTIVO: el PDF adjunto es este mismo
	...
	function documentoMesActivo() {
		const d = datosMesActivo;
		if (!d || (!d.cabecera && !d.items.length && !d.adicionales.length)) return null;
		return documentoOcasional({
	...
	async function exportarLibroExcel() {
		...
		try {
			await exportarExcelLibro('ocasional', hojas, nombreArchivoAnual('OCASIONALES', anio));
	...
	async function exportarZipPdf() {
		...
		for (let m = 1; m <= 12; m++) {
			...
			if (!d.cabecera && !d.items.length && !d.adicionales.length) continue;
			hojas.push({
				nombreArchivo: nombreArchivoHoja('OCASIONALES', m, anio),
				documento: documentoOcasional({
					cabecera: d.cabecera,
					items: d.items,
					adicionales: d.adicionales,
		...
		try {
			const { generados, fallidas } = await exportarZipPdfs(
				'ocasional',
				hojas,
				nombreArchivoAnual('OCASIONALES', anio),
				{
...
<UniverToolbar
	title="LIQUIDACIONES OCASIONALES"
	hoja={periodDisplay}
...
>
	{#snippet actions()}
		...
		<!-- Las ACCIONES viven en el carril derecho, no aquí.
		     Eran ocho botones en una fila que se recorta en silencio al
		     desbordar: con el selector de año, el de mes, los avatares y el
		     indicador de guardado, «Exportar Excel» y «Cerrar y distribuir»
		     caían fuera de pantalla en un portátil. El carril las apila y cada
		     una explica lo que hace al pasar por encima, que es lo que el
		     `title` de un botón sin texto no llegaba a decir. Mismo patrón que
		     el canvas de placas. -->
...
{#if modalTraerItems}
	<ItemsDisponiblesOcasionalModal
		mes={mesActivo}
		{anio}
		periodo={periodDisplay}
		onClose={() => (modalTraerItems = false)}
		onAgregado={async (n) => {
			// Recarga el mes y remonta: la tabla de items gana filas, y con ellas
			// se desplazan el pie, los totales y el ancla de la zona libre.
			await recargarMes(mesActivo);
			toast.success(`${n} item(s) añadidos a ${periodDisplay}`);
		}}
	/>
{/if}
...
{#snippet icoTraerItems()}
	<!-- Bandeja con flecha ENTRANDO. Misma que en el canvas de placas: las dos
	     hacen lo mismo —traer filas que ya existen— y compartir silueta ayuda a
	     encontrarla al saltar de un canvas a otro. -->
	<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
...
{#snippet icoDocumento()}
	<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
		...
		<path d="M19.5 9.75v8.5a2.25 2.25 0 01-2.25 2.25H6.75A2.25 2.25 0 014.5 18.25V5.75A2.25 2.25 0 016.75 3.5h7.5l5.25 6.25z" />
	</svg>
{/snippet}
...
<div class="oc-body">
	...
	<UniverSideRail
		ariaLabel="Acciones de liquidaciones ocasionales"
		items={[
			{
				id: 'generar',
				label: cabeceraActiva ? 'Refrescar items' : 'Generar borrador',
				hint: cabeceraActiva
			...
			{
				id: 'filas',
				label: 'Filas del ocasional',
				hint: 'Añadir o quitar filas de gastos de vehículo, anticipos e impuestos de este mes.',
				icon: icoFilas,
				badge: gastosExtraMes.length + anticiposMes.length || null,
				disabled: !cabeceraActiva,
				disabledHint: `${periodDisplay} no tiene borrador: genéralo antes de añadir filas.`,
				onSelect: () => (modalGastosOpen = true)
			},
			{
				id: 'traer-items',
				label: 'Traer items',
				hint: 'Buscar servicios facturados que sigan sueltos —de cualquier mes, anterior o posterior— y añadirlos a este ocasional sin regenerar el borrador.',
				icon: icoTraerItems,
				disabled: !cabeceraActiva,
				disabledHint: `${periodDisplay} no tiene borrador: genéralo antes de traer items.`,
			...
			},
			{
				...
				hint: `Abrir el documento guardado en el servidor para ${periodDisplay}. Es el que se entregó, no lo que hay en pantalla.`,
				icon: icoDocumento,
				disabled: !cabeceraActiva,
				disabledHint: `${periodDisplay} no tiene borrador.`,
				onSelect: verDocumentoOficial
			},
			{
				id: 'historial',
				label: 'Historial',
				hint: `Ver y restaurar versiones de ${periodDisplay}.`,
				icon: icoHistorial,
			...
			},
			{
				...
				hint: `Descargar un .xlsx de ${anio} con una pestaña por mes.`,
				icon: icoExcel,
			...
			},
			{
				...
				hint: `Descargar un ZIP con un PDF por mes de ${anio}.`,
				icon: icoZip,
				busy: exportandoZip,
			...
			...(cabeceraActiva?.estado === 'BORRADOR'
				? [
						{ type: 'sep' as const },
						{
							id: 'cerrar',
							label: 'Cerrar y distribuir',
							hint: `Cerrar ${cabeceraActiva.consecutivo} y repartirlo a los terceros. Deja de ser BORRADOR.`,
							icon: icoCerrar,
							tone: 'green' as const,
							onSelect: cerrarYDistribuir
						}
					]
				: [])
		]}
...
{#if envioAbierto}
	<EnvioHojaModal
		...
		{anio}
		mes={mesActivo}
		hojaLabel="OCASIONALES"
		tituloCorreo="Liquidación ocasional"
		nombreArchivo={nombreArchivoHoja('OCASIONALES', mesActivo, anio)}
		resumen={[{ etiqueta: 'Total a pagar', valor: `$${formatCOP(totalPagarMes)}` }]}
		origenId={cabeceraActiva?.id ?? null}
		obtenerDocumento={documentoMesActivo}
...
<style>
	/* Fila: canvas elástico + carril de 32px. Copia deliberada del layout del
	   canvas de placas: `.oc-canvas` es COLUMNA para que `.univer-host` conserve
	   el padre flex-column del que cuelga su cadena de altura, y `min-width: 0`
	   evita que el `width:100%` del host empuje al carril fuera de pantalla. */
	.oc-body {
		flex: 1 1 auto;
		min-height: 0;
		display: flex;
