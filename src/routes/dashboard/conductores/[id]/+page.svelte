<script lang="ts">
	...
	async function handleSubmit() {
		try {
			...
			// Ocultar después de 2.5s y recargar
			setTimeout(async () => {
	...
	async function handleUploadCroppedImage() {
		try {
			...
			const croppedFile = new File([croppedBlob], imageFile?.name || 'photo.jpg', { type: 'image/jpeg' });
	...
	function getEstadoBadgeClass(estado: string): string {
		switch (estado?.toUpperCase()) {
			...
			case 'RETIRADO': return 'bg-red-100 text-red-800 border border-red-200';
			default: return 'bg-gray-100 text-gray-800 border border-gray-200';
		}
	}

	const tabs = [
		{ id: 'personal' as TabType, label: 'Información Personal', icon: 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z' },
		{ id: 'laboral' as TabType, label: 'Información Laboral', icon: 'M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z' },
		{ id: 'seguridad' as TabType, label: 'Seguridad Social', icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' },
		{ id: 'licencia' as TabType, label: 'Licencia', icon: 'M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2' }
	];
</script>
...
>
	<div class="w-full px-4 py-6 sm:px-6 lg:px-8">
		...
		<div class="page-card mb-6" style="padding: 1.25rem 1.5rem;">
			<button
						...
						<div class="flex-1 text-center md:text-left">
							...
							<div class="mt-3 flex flex-wrap justify-center gap-2 md:justify-start">
								...
								{#if conductor.sede_trabajo}
									<span class="rounded-full border border-white/30 bg-white/20 px-3 py-1 text-xs font-medium text-white">{conductor.sede_trabajo}</span>
								{/if}
							</div>
							<div class="mt-4 flex flex-wrap justify-center gap-2 md:justify-start">
								<button
									...
									class="inline-flex items-center rounded-lg border border-white bg-white/10 px-3 py-2 text-sm font-medium text-white backdrop-blur-sm transition-colors hover:bg-white/20"
									title="Ver recorridos y bonos de planilla"
								>
									<svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8">
										<path stroke-linecap="round" stroke-linejoin="round" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" />
									</svg>
									Ver recorridos
								</button>
								<label class="inline-flex cursor-pointer items-center rounded-lg border border-white bg-white/10 px-3 py-2 text-sm font-medium text-white backdrop-blur-sm transition-colors hover:bg-white/20">
									<svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
									{conductor.foto_signed_url ? 'Cambiar foto' : 'Subir foto'}
									<input type="file" accept="image/*" on:change={handleFileSelect} class="hidden" disabled={isUploadingPhoto} />
								</label>
								{#if conductor.foto_signed_url}
									<button on:click={handleDeletePhoto} disabled={isUploadingPhoto} class="inline-flex items-center rounded-lg border border-white/50 bg-red-500/80 px-3 py-2 text-sm font-medium text-white backdrop-blur-sm transition-colors hover:border-red-400 disabled:opacity-50">
...
{#if showSuccessAnim}
	<div class="success-overlay" in:fade={{duration: 200}} out:fade={{duration: 300}}>
		<div class="success-content" in:scale={{duration: 400, start: 0.8}}>
			<div class="success-circle">
				<svg class="success-check" viewBox="0 0 52 52">
					<circle class="success-circle-bg" cx="26" cy="26" r="24" fill="none" stroke="#fff" stroke-width="2" />
					<path class="success-check-path" fill="none" stroke="#fff" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round" d="M14 27l7.8 7.8L38 18" />
				</svg>
			</div>
...
<!-- Crop Modal -->
{#if showCropModal}
	<div class="fixed inset-0 z-50 overflow-y-auto" transition:fade={{ duration: 200 }} role="dialog" aria-modal="true">
		<div class="flex min-h-screen items-center justify-center px-4 pt-4 pb-20 text-center sm:p-0">
			...
			<div class="relative inline-block transform overflow-hidden rounded-lg bg-white text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-3xl sm:align-middle" transition:fly={{ y: 50, duration: 300 }}>
				<div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
					<div class="mb-4 flex items-center justify-between"><h3 class="text-lg leading-6 font-medium text-gray-900">Recortar Foto de Perfil</h3><button type="button" on:click={handleCloseCropModal} class="text-gray-400 hover:text-gray-500"><svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg></button></div>
					<div class="relative h-96 w-full overflow-hidden rounded-lg bg-gray-900">
						<Cropper image={imageSrc} bind:crop bind:zoom aspect={undefined} oncropcomplete={onCropComplete} />
					</div>
					<div class="mt-4 space-y-4">
						<div><label for="zoom" class="mb-2 block text-sm font-medium text-gray-700"> Zoom </label><input id="zoom" type="range" min="1" max="3" step="0.1" bind:value={zoom} class="h-2 w-full cursor-pointer appearance-none rounded-lg bg-gray-200" /></div>
						<div><label for="rotation" class="mb-2 block text-sm font-medium text-gray-700"> Rotación </label><input id="rotation" type="range" min="0" max="360" step="1" bind:value={rotation} class="h-2 w-full cursor-pointer appearance-none rounded-lg bg-gray-200" /></div>
					</div>
				</div>
...
<style>
	...
	.success-check { width: 65px; height: 65px; }
	.success-check-path {
		stroke-dasharray: 48;
		stroke-dashoffset: 48;
		animation: successCheckDraw 0.45s cubic-bezier(0.65, 0, 0.45, 1) 0.5s forwards;
	}
