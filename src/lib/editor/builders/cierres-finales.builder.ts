...
const COLS_SUMADAS = [
	...
	COL.ING_EXTRA_GLOBAL, COL.ING_EXTRAS_AVAL, COL.ING_TRANSMERALDA
];
...
export interface CopropietarioCierre {
	id: string;
	/**
	 * Ficha del catálogo `terceros`, si el copropietario está dado de alta.
	 *
	 * Es opcional en la base: un copropietario puede existir solo como texto
	 * libre. Sin ficha no hay correo por defecto ni dónde persistir el que se
	 * corrija en el envío, y el modal lo dice en vez de callarlo.
	 */
	tercero_id: string | null;
	/** Correo de esa ficha. Destinatario por defecto de SU liquidación. */
	correo: string | null;
	nombre: string;
	identificacion: string | null;
	/** Porcentaje declarado (visual); puede sumar ≠ 100% entre todos. */
	porcentaje: number;
	/**
	 * Porcentaje efectivo de la cascada por orden (ver
	 * `$lib/editor/business/reparto-propietarios.ts`). Con él se calculan
	 * valor a facturar, impuestos y descuentos del propietario.
	 */
	...
	/** Posición en la cascada (menor = primero). */
	orden: number;
}
...
export interface AnclasHoja {
	...
	/// `null` en hojas multi-propietario: ahí no se pinta el resumen final,
	/// porque el neto es de cada copropietario y no de la hoja.
...
export function buildCierreFinalSheet(
	...
	// Ámbar y no morado: la sección de copropietarios comparte familia con los
	...
	const coproSubHeader = subHeader('#B45309', '#FFEDD5');

	// Pagos internos por concepto: verde CLARO. Ni ámbar (que en esta hoja
	...
	items.forEach((it, idx) => {
		...
		if (!it.excluido) {
			...
			sums[COL.ING_TRANSMERALDA] += ingresoTransmeralda;
		}
		row++;
	});
	...
	adicionales.forEach((a, idx) => {
		...
		sums[COL.ING_TRANSMERALDA] += ingresoAdicional;
	...
	// Totales que necesitan tanto la sección de copropietarios (reparto por
	// propietario) como los TOTALES FINALES de abajo.
	...
	/// Una hoja multi-propietario liquida a varias personas a la vez: no
	/// tiene un total único al pie (ver sección 7).
	const esMultiPropietario = hoja.es_multi_propietario && copropietarios.length > 0;

	if (esMultiPropietario) {
		anclaCopro = row;

		// La cascada la calcula y persiste el servidor (`porcentaje_efectivo`);
		// aquí solo se REPARTE con ella, con el mismo espejo del preview.
		//
		// Aritmética del reparto: los descuentos generales (nómina + gastos +
		// anticipos) se totalizan ANTES de repartir; lo que queda es el VALOR
		// SERVICIO, y ese es el que se divide como valor a facturar. Las
		// retenciones son de cada propietario (las calcula y persiste el
		// servidor sobre SU facturar); un propietario sin retenciones (pago
		// interno por concepto, ej. abono a crédito) recibe su proporcional
		// tal cual.
		const coproOrdenados = [...copropietarios].sort((a, b) => a.orden - b.orden);
		const propsEfectivos = coproOrdenados.map((p) => ({
			id: p.id,
			efectivo: n(p.porcentaje_efectivo),
			orden: p.orden
		}));
		const descuentosGenerales = totalCostosLaborales + totalGastos + totalAnticipos;
		const valorServicioReparto = totalServicio - descuentosGenerales;
		const facturarMap = repartirValor(valorServicioReparto, propsEfectivos);
		const impuestosDe = (pid: string) =>
			impuestos
				.filter((x) => (x as any).propietario_id === pid)
				.reduce((s, x) => s + n(x.valor_total), 0);
		const conRetenciones = coproOrdenados.filter((p) => p.aplica_retenciones);
		const sinRetenciones = coproOrdenados.filter((p) => !p.aplica_retenciones);
		...
		// Totalizado general ANTES de las tablas de propietarios: cuánto se
		// descuenta del bruto y cuánto queda listo para repartir.
		const filaKv = (etiqueta: string, valor: number, estiloValor: IStyleData) => {
			set(row, 0, etiqueta, secTfStyle);
			merge(row, 0, 3);
			soloEstilo(row, 1, 3, secTfStyle);
			cellData[row][0] = { v: etiqueta, t: CellValueType.STRING, s: secTfStyle };
			set(row, 4, valor, {
				...estiloValor, ht: HorizontalAlign.RIGHT, n: { pattern: '"$"#,##0' }
			} as any);
			row++;
		};
		filaKv('TOTAL DESCUENTOS', descuentosGenerales,
			{ ...secTfStyle, cl: { rgb: RED }, bg: { rgb: RED_SOFT } } as any);
		filaKv('VALOR SERVICIO (LISTO PARA REPARTIR)', valorServicioReparto,
			{ ...secTfStyle, bl: 1 } as any);

		const sumaPct = coproOrdenados.reduce((s, p) => s + n(p.porcentaje), 0);
		...
		set(row, 4, 'VALOR A FACTURAR', { ...coproSubHeader, ht: HorizontalAlign.RIGHT });
		rowData[row] = { h: 22 };
		row++;

		// La nota es el concepto de pago del propietario (ej. "ABONAR A CRÉDITO
		// BANCOOMEVA"): viaja pegada al nombre para que el valor no quede huérfano.
		const nombreConNota = (p: CopropietarioCierre) =>
			p.nota ? `${p.nombre} — ${p.nota}` : p.nombre;

		for (const p of coproOrdenados) {
			set(row, 0, nombreConNota(p), nominaCellBase);
			merge(row, 0, 1);
			if (!cellData[row]) cellData[row] = {};
			cellData[row][1] = { s: nominaCellBase };
			set(row, 2, p.identificacion || '—', nominaCellBase);
			// % declarado (visual); si la cascada lo cambia, el efectivo va al lado.
			const pctVisual = `${n(p.porcentaje).toFixed(2)}%`;
			const difiere = Math.abs(n(p.porcentaje_efectivo) - n(p.porcentaje)) > 0.005;
			set(row, 3, difiere
				? `${pctVisual} (ef. ${n(p.porcentaje_efectivo).toFixed(2)}%)`
				: pctVisual,
				{ ...nominaCellBase, ht: HorizontalAlign.RIGHT });
			set(row, 4, facturarMap.get(p.id) ?? 0, {
				...nominaCellBase, ht: HorizontalAlign.RIGHT, n: { pattern: '"$"#,##0' }
			} as any);
			row++;
		}
		...
		// corregirlo — el reparto real es la CASCADA por orden: el primero
		// toma su % del total y los demás reparten el remanente.
		const avisa = Math.abs(sumaPct - 100) > 0.01;
		const notaSuma = avisa
			? `SUMA DE PARTICIPACIONES: ${sumaPct.toFixed(2)}%  (cálculo en cascada por orden)`
		...
		// ── Matriz retenciones × copropietario (solo quienes tributan) ──
		// La liquidación individual: su valor a facturar, sus retenciones
		// concepto a concepto (calculadas por el servidor sobre SU facturar) y
		// su neto. Los propietarios sin retenciones van aparte, a lo ancho.
		const moneda = { ht: HorizontalAlign.RIGHT, n: { pattern: '"$"#,##0' } };
		// El CONCEPTO ocupa A:B (los nombres de fila no caben en una columna) y
		// los propietarios arrancan en la C.
		const COL_PRIMER_COPRO = 2;
		const etiquetaMatriz = (r: number, texto: string, estilo: IStyleData) => {
			set(r, 0, texto, estilo);
			merge(r, 0, 1);
			if (!cellData[r]) cellData[r] = {};
			cellData[r][1] = { s: estilo };
		};

		if (conRetenciones.length > 0) {
			set(row, 0, 'RETENCIONES Y NETO A PAGAR POR COPROPIETARIO', coproBand);
			const ultimaColMatriz = Math.min(
				COL_PRIMER_COPRO + conRetenciones.length - 1,
				TOTAL_COLS - 1
			);
			merge(row, 0, ultimaColMatriz);
			soloEstilo(row, 1, ultimaColMatriz, coproBand);
			cellData[row][0] = {
				v: 'RETENCIONES Y NETO A PAGAR POR COPROPIETARIO',
				t: CellValueType.STRING, s: coproBand
			};
			rowData[row] = { h: 22 };
			row++;

			etiquetaMatriz(row, 'CONCEPTO', coproSubHeader);
			conRetenciones.forEach((p, i) => {
				set(row, COL_PRIMER_COPRO + i, nombreConNota(p), {
					...coproSubHeader, ht: HorizontalAlign.RIGHT
				});
			});
			rowData[row] = { h: 22 };
			row++;

			etiquetaMatriz(row, 'VALOR A FACTURAR', nominaCellBase);
			conRetenciones.forEach((p, i) => {
				set(row, COL_PRIMER_COPRO + i, facturarMap.get(p.id) ?? 0, {
					...nominaCellBase, ...moneda
				} as any);
			});
			row++;
			...
			const matrizImpIni = row;
			for (const nombreConcepto of conceptosImp) {
				etiquetaMatriz(row, `(−) ${nombreConcepto.replace(/_/g, ' ')}`, nominaCellBase);
				conRetenciones.forEach((p, i) => {
					const fila = impuestos.find(
						(x) => x.concepto === nombreConcepto && (x as any).propietario_id === p.id
					);
					set(row, COL_PRIMER_COPRO + i, n(fila?.valor_total), {
						...nominaCellBase, ...moneda
					} as any);
					if (fila?.id) {
						bindDerivada(row, COL_PRIMER_COPRO + i, {
			...
			etiquetaMatriz(row, '(−) TOTAL RETENCIONES', { ...secTfStyle, cl: { rgb: RED } } as any);
			conRetenciones.forEach((p, i) => {
				set(row, COL_PRIMER_COPRO + i, impuestosDe(p.id), {
					...secTfStyle, cl: { rgb: RED }, bg: { rgb: RED_SOFT }, ...moneda
				} as any);
			});
			row++;
		...
		// ── Pagos internos por concepto (sin retenciones) ───────────────
		// Antes cada uno se pintaba suelto, en ámbar y con la nota repetida
		// debajo: quedaba mezclado con los avisos de la cascada (que también
		// son ámbar) y con los propietarios que SÍ cobran. Ahora es un bloque
		// propio, en verde claro, con su cabecera, su columna de valor y un
		// solo pie explicativo: se lee de un vistazo cuánto del reparto se
		// destina a conceptos y cuánto sale de verdad hacia un propietario.
		if (sinRetenciones.length > 0) {
			row++;

			set(row, 0, 'DESTINADO A CONCEPTO (PAGO INTERNO)', conceptoBand);
			merge(row, 0, 4);
			soloEstilo(row, 1, 4, conceptoBand);
			cellData[row][0] = {
				v: 'DESTINADO A CONCEPTO (PAGO INTERNO)',
				t: CellValueType.STRING, s: conceptoBand
			};
			rowData[row] = { h: 22 };
			row++;

			set(row, 0, 'CONCEPTO', conceptoSubHeader);
			merge(row, 0, 2);
			soloEstilo(row, 1, 2, conceptoSubHeader);
			cellData[row][0] = {
				v: 'CONCEPTO', t: CellValueType.STRING, s: conceptoSubHeader
			};
			set(row, 3, '%', { ...conceptoSubHeader, ht: HorizontalAlign.RIGHT });
			set(row, 4, 'VALOR PROPORCIONAL',
				{ ...conceptoSubHeader, ht: HorizontalAlign.RIGHT });
			rowData[row] = { h: 22 };
			row++;

			// Sin totalizador, a propósito: cada concepto es un destino
			// independiente —un crédito, un fondo— y sumarlos da una cifra que
			// no se paga a nadie ni se concilia contra nada. Lo que se consulta
			// es cuánto va a CADA concepto, no cuánto va «a conceptos».
			for (const p of sinRetenciones) {
				// El CONCEPTO es el protagonista de la fila (ej. "ABONAR A
				// CRÉDITO BANCOOMEVA"); el propietario que lo recibe va debajo,
				// en pequeño, porque aquí el dato que se consulta es a qué se
				// destina el dinero, no quién figura como titular.
				const concepto = (p.nota || p.nombre).toUpperCase();
				const valor = facturarMap.get(p.id) ?? 0;

				set(row, 0, concepto, conceptoFilaFuerte);
				merge(row, 0, 2);
				soloEstilo(row, 1, 2, conceptoFilaFuerte);
				cellData[row][0] = {
					v: concepto, t: CellValueType.STRING, s: conceptoFilaFuerte
				};
				set(row, 3, `${n(p.porcentaje).toFixed(2)}%`,
					{ ...conceptoFila, ht: HorizontalAlign.RIGHT });
				set(row, 4, valor,
					{ ...conceptoFilaFuerte, ...moneda } as any);
				row++;

				const difiereConcepto =
					Math.abs(n(p.porcentaje_efectivo) - n(p.porcentaje)) > 0.005;
				const titular = difiereConcepto
					? `${p.nombre} · ${n(p.porcentaje).toFixed(2)}% (ef. ${n(p.porcentaje_efectivo).toFixed(2)}%)`
					: p.nombre;
				set(row, 0, titular, conceptoFilaNota);
				merge(row, 0, 4);
				soloEstilo(row, 1, 4, conceptoFilaNota);
				cellData[row][0] = {
					v: titular, t: CellValueType.STRING, s: conceptoFilaNota
				};
				row++;
			}

			const pieConcepto =
				'Pago interno: se descuenta del reparto y se abona al concepto. No genera egreso real ni tributa.';
			set(row, 0, pieConcepto, conceptoPie);
			merge(row, 0, 4);
			soloEstilo(row, 1, 4, conceptoPie);
			cellData[row][0] = {
				v: pieConcepto, t: CellValueType.STRING, s: conceptoPie
			};
			row++;
		}
		row++;
	...
	// En MULTI-PROPIETARIO este resumen se omite entero.
	//
	// No hay un «total a pagar» de la hoja: el bruto se reparte en cascada y
	// cada copropietario tiene su propio neto —con sus retenciones— en la
	// matriz de arriba, y lo que va a conceptos ni siquiera se paga. Un
	// TOTAL A PAGAR único al pie invitaba a leerlo como el cheque de la
	// placa, cuando en realidad es la suma de cosas que se pagan a personas
	// distintas más un abono interno. El documento cierra en el reparto.
	let filaTotalDescuentos: number | null = null;
	let filaTotalPagar: number | null = null;

	if (!esMultiPropietario) {
		// Local no-nulable: la fórmula de TOTAL A PAGAR referencia esta fila.
		const filaDescuentos = row;
		filaTotalDescuentos = filaDescuentos;
		...
		const sumandosDescuentos = filasTotalizadoras.map((f) => `E${f}`);
		setFormula(
			row,
			4,
			sumandosDescuentos.length ? '=' + sumandosDescuentos.join('+') : '=0',
			totalDescuentos,
			redTotalValue
		);
		rowData[row] = { h: 28 };
		...
		setFormula(row, 4, `=${refServicio}-E${filaDescuentos + 1}`, totalPagar, greenTotalValue);
		rowData[row] = { h: 28 };
		row++;
	}
