...
export function fondoFilaIngresos(o: {
	...
	zebra: boolean;
	columna: number;
	/// La fila es un ADICIONAL de un cierre final, no un servicio facturado.
	adicional?: boolean;
}): string {
	if (o.incluida) return o.prioritaria ? AZUL_PRIORIDAD : VERDE_INCLUIDA;
	// El ámbar va DEBAJO del verde de INCLUIR a propósito: cuando el adicional
	// baja a la hoja de adicionales, lo que importa saber de un vistazo es que
	// bajó, igual que en cualquier otra fila. Que era un adicional lo sigue
	// diciendo el texto de DESCRIPCION, que no depende del color y sobrevive a
	// imprimir la hoja.
	if (o.adicional) return AMBAR_ADICIONAL;
...
function construirHoja(input: HojaInput): HojaSalida {
	...
	filas.forEach((f, idx) => {
		...
		const adicional = esAdicional(f.item);
		 ...
		 *
		 * La excepción son los ADICIONALES: ahí sí distingue, porque en esa
		 * hoja conviven dos cosas que se liquidan al revés —un servicio que
		 * baja con su % de ganancia y un pago ya hecho que entra entero y en
		 * negativo—, y el ámbar es lo que las separa de un vistazo.
		 */
		const conFondo = (s: IStyleData, columna: number): IStyleData => {
			if (!esIngresos && !adicional) return s;
			return {
				...s,
				bg: {
					rgb: fondoFilaIngresos({
						prioritaria,
						// En ADICIONALES no se pinta el verde de INCLUIR (todo lo
						// visible está marcado), así que la fila de adicional se
						// queda con su ámbar en vez de perderlo bajo el verde.
						incluida: esIngresos && f.incluida,
						zebra,
						columna,
						adicional
					})
		...
		// Cero en las filas de ADICIONAL: la administración es lo que la empresa
		// se queda de lo que le ENTRA, y un adicional sale. El porqué largo está
		// en `pctAdmonDeFila`, que es la única implementación de esta regla —la
		// hoja y los totales del subtítulo tienen que decir lo mismo.
		const pctAdmon = pctAdmonDeFila(f.item, f.estado, pct, hoja);
