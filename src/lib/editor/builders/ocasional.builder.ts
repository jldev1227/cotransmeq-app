...
export function buildOcasionalSheet(input: OcasionalSheetInput): any {
	...
	// ─── SECCIÓN 5: ANTICIPOS ───────────────────────────────────────────────
	// FALTABA. `TOTAL DESCUENTOS` ya sumaba los conceptos `ANTICIPO` —la línea
	// está unas cuantas más abajo— pero ninguna sección los pintaba, así que un
	// anticipo restaba del TOTAL A PAGAR sin aparecer en la hoja: el documento
	// cuadraba consigo mismo y no había forma de saber de dónde salía la
	// diferencia. Nadie lo había notado porque hasta ahora no existía interfaz
	// para crear uno; en cuanto el carril la abre, deja de ser teórico.
	//
	// A diferencia de GASTOS, aquí NO hay filas fijas: un anticipo es un hecho
	// suelto —una fecha, un importe— y no un concepto recurrente del formato.
	// Si el mes no tiene ninguno, la sección no se pinta: un bloque con una
	// única fila «(sin anticipos)» ocupa lo mismo que tres reales y no dice
	// nada que el TOTAL DESCUENTOS no diga ya.
	{
		...
		if (anticipos.length > 0) {
			const rows = anticipos.map((c) => {
				const cant = Number(c.dias ?? 1);
				const vu = Number(c.valor_unitario ?? 0);
				return [c.concepto, cant, vu, Number(c.valor_total || 0) || cant * vu] as [
					string,
					string | number,
					string | number,
					string | number
			...
			const metas = anticipos.map((c) =>
				c?.id
					? {
							conceptoId: c.id,
							colAField: 'concepto' as const,
							colCField: 'dias' as const,
							colDField: 'valor_unitario' as const
						}
					: undefined
			);
			const total = rows.reduce((s2, row) => s2 + Number(row[3] || 0), 0);
			writeSmallSection(
				'ANTICIPOS',
				BLUE,
				['CONCEPTO', '', 'CANTIDAD', 'VALOR', 'TOTAL'],
