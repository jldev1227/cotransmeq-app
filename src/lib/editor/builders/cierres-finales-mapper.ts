...
function copropietariosDeDetalle(d: any): CopropietarioCierre[] {
	const filas: any[] = Array.isArray(d.propietarios) ? d.propietarios : [];
	// Fallback para respuestas de un backend sin `porcentaje_efectivo`
	// (anterior a la cascada): se calcula aquí con el espejo local.
	const efectivos = calcularPorcentajesEfectivos(
		filas.map((p: any, idx: number) => ({
			id: p.id,
	...
	return filas.map(
		(p: any, idx: number): CopropietarioCierre => ({
			id: p.id,
			tercero_id: p.tercero_id ?? p.tercero?.id ?? null,
			// El correo de la ficha, no uno guardado en el copropietario: la
			// dirección de una persona es del catálogo y así una corrección
			// vale para todas sus placas, no solo para esta hoja.
			correo: txt(p.tercero?.correo) || null,
			nombre: txt(p.nombre || p.tercero?.nombre_completo),
			identificacion: p.identificacion ?? p.tercero?.identificacion ?? null,
			porcentaje: num(p.porcentaje),
			porcentaje_efectivo:
				p.porcentaje_efectivo != null ? num(p.porcentaje_efectivo) : (efectivos.get(p.id) ?? 0),
			nota: p.nota ?? null,
