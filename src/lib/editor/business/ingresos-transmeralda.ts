...
/// Cuántas filas LIBRES abre cada bloque del pie. Ver `SEMILLAS`.
const GASTOS_LIBRES = 4;
const ANTICIPOS_LIBRES = 2;
...
function semillasIngresos(): SemillaIngreso[] {
	...
	for (const hoja of ['INGRESOS', 'ADICIONALES'] as HojaIngreso[]) {
		...
		for (let i = 1; i <= GASTOS_LIBRES; i++) {
			out.push({
				hoja,
				tipo: 'GASTO_OPERATIVO',
				clave: `GASTO_LIBRE_${i}`,
				concepto: `GASTO ${i}`,
				porcentaje: null,
				orden: 1 + i
			});
		}
		// Bloque «IMPUESTOS Y GASTOS DEL VEHÍCULO».
		out.push({
			hoja,
			tipo: 'ANTICIPO',
			clave: 'OTROS_ANTICIPOS',
			concepto: 'OTROS_ANTICIPOS',
			porcentaje: null,
			orden: 10
		});
		for (let i = 1; i <= ANTICIPOS_LIBRES; i++) {
			out.push({
				hoja,
				tipo: 'ANTICIPO',
				clave: `ANTICIPO_LIBRE_${i}`,
				concepto: `ANTICIPO ${i}`,
				porcentaje: null,
				orden: 10 + i
			});
	...
	// Los impuestos solo existen en la hoja de ADICIONALES: la de INGRESOS no
	// retiene nada, su bloque de abajo son anticipos y gastos.
...
export function calcularFilasIngresos(
	...
	return items.map((item) => {
		...
		const admon = roundExcel(
			(total * (estado?.pct_admon_ingresos ?? pct.admonIngresos)) / 100
		);
		return {
			item,
			estado,
...
export function calcularFilasAdicionalesTodas(
	...
	return items.map((item) => {
		...
		const total = vUnidad * cantidad;
		const admon = roundExcel(
			(total * (estado?.pct_admon_adicional ?? pct.admonAdicionales)) / 100
		);
		return {
			item,
			estado,
