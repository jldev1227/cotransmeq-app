...
/**
 * Los gastos de vehículo que la hoja SIEMPRE pinta.
 *
 * El orden es el de la tabla; `writeSmallSection` los renderiza en este mismo
 * orden aunque no exista ninguno en la base.
 *
 * ERAN CINCO. DOTACION, EXAMEN_MEDICO y COMBUSTIBLE salieron de aquí: no son
 * del formato, son gastos que unos meses hay y otros no, y sembrarlos dejaba
 * tres filas en cero que se imprimían en el PDF y se exportaban al Excel. Ahora
 * se añaden desde «Filas del ocasional» cuando toca, igual que cualquier otro.
 *
 * PAPELERIA y GASTOS_DIVERSOS se quedan porque sí son del formato: van en todos
 * los meses y el documento los espera aunque valgan cero.
 *
 * Lo ya guardado NO se pierde: un DOTACION de un mes anterior sigue viniendo de
 * la base y el builder lo pinta como fila añadida —con su nombre editable y su
 * botón de quitar— en vez de como fila fija.
 */
export const GASTOS_VEHICULO: string[] = ['PAPELERIA', 'GASTOS_DIVERSOS'];
