...
export interface EngineOptions {
  ...
  /**
   * Autofiltro tipo Excel: el desplegable en cada celda de cabecera.
   *
   * Opt-in por el mismo motivo que `dataValidation`: el plugin añade su UI a
   * la cabecera y sus entradas al menú contextual, y las hojas del editor
   * clásico (una liquidación, no un listado) no tienen nada que filtrar. El
   * RANGO no lo pone el preset — hay que crearlo con `createFilter()` sobre
   * la cabecera y los datos; ver `servicios-historial-engine.ts`.
   */
  filtros?: boolean;
}
    ...
    locales: {
      [locale]: withForceStringInfoPatch(
        ...
        mergeLocales(
          ...
          UniverPresetSheetsFilterEsES as unknown as Record<string, any>
        )
      )
    },
    presets: [
      ...(opts.filtros ? [UniverSheetsFilterPreset()] : []),
      ...(opts.dataValidation
        ? [
