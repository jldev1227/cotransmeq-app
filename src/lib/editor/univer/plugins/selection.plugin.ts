...
function rangosDeLaHoja(fUniver: FUniver): IRange[] {
	...
	console.log('[dbg] ws?', !!ws, 'sel?', !!sel,
		'lista=', JSON.stringify((() => { const l = sel?.getActiveRangeList?.(); return Array.isArray(l) ? 'array:'+l.length : (typeof l?.getRanges === 'function' ? 'obj:'+l.getRanges().length : typeof l); })()),
		'selActive=', JSON.stringify(sel?.getActiveRange?.()?.getRange?.() ?? null),
		'wsActive=', JSON.stringify(ws.getActiveRange?.()?.getRange?.() ?? null));
...
export function attachSelectionPlugin(opts: SelectionPluginOptions): () => void {
	...
	function leer() {
		...
		ultimaClave = clave;
		console.log('[dbg] onChange ->', JSON.stringify(filas));
		opts.onChange(filas);
	}

	function programarLectura() {
		console.log('[dbg] programarLectura, programado=', programado);
	...
	console.log('[dbg] attach en', opts.container?.className, opts.container?.isConnected);
	opts.container.addEventListener('pointerup', programarLectura);
	opts.container.addEventListener('keyup', programarLectura);
	document.addEventListener('pointerup', () => console.log('[dbg] pointerup en document'), { capture: true });
