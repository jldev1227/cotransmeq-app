/**
 * Generador de PDF Desprendible de Intereses de Cesantías usando pdfmake
 * Portado de pdfMakerInteresesCesantias.tsx
 */
...
export async function generarPdfInteresesCesantias(
	item: Liquidacion,
	firmas: FirmaConUrl[] = []
): Promise<void> {
	...
	const content: any[] = [
			...
			columns: [
				{
					stack: [
						...
						{
							text: 'DESPRENDIBLE DE INTERESES DE CESANTÍAS',
							fontSize: 10,
							color,
							bold: true,
							margin: [0, 8, 0, 0]
		...
		// Datos del empleado
		{
			table: {
				...
				body: [
					...
					[{ text: 'Periodo' }, { text: 'Año 2025', alignment: 'right' as const }]
				]
			},
			layout: {
		...
		{
			text: 'DETALLE DE INTERESES DE CESANTÍAS',
			bold: true,
			color,
			fontSize: 11,
			margin: [0, 15, 0, 6]
		...
		// Información destacada
		{
					...
					[
						{
							stack: [
								...
								{
									text: 'Este desprendible corresponde al pago de los intereses de cesantías correspondientes al año 2025. Los valores que se detallan a continuación fueron cancelados dentro de los términos legales establecidos y se presentan en este documento para su información y registro.',
									fontSize: 9,
									lineHeight: 1.4
								}
							]
			...
			layout: {
				hLineWidth: () => 1,
				vLineWidth: () => 1,
				hLineColor: () => (esCotransmeq ? '#FFA726' : '#91D5FF'),
				vLineColor: () => (esCotransmeq ? '#FFA726' : '#91D5FF'),
				fillColor: () => (esCotransmeq ? '#FFF4E6' : '#E6F4FF'),
				paddingLeft: () => 10,
				paddingRight: () => 10,
				paddingTop: () => 8,
				paddingBottom: () => 8
			...
			table: {
				...
				body: [
					[
						{
							stack: [
								{ text: 'Intereses de cesantías' },
								{
									text: 'Calculado sobre el saldo de cesantías al 31 de diciembre',
									fontSize: 8,
									color: '#666',
									italics: true
								}
						...
						{
							text: formatCurrency(Number(safeValue(item.interes_cesantias, 0))),
							color,
