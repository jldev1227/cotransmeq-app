	...
	if (recargosData?.planillas && recargosData.planillas.length > 0) {
		...
		for (const planilla of planillas) {
			...
			if (tiposConsolidados.length > 0) {
				...
				// Bono festivo (if applicable)
				if (
					planilla.configuracion_salarial?.paga_dias_festivos &&
					planilla.total_festivos > 0
				) {
					const diasFestivos = dias.filter(
						(d: any) => !d.disponibilidad && (d.es_festivo || d.es_domingo)
					).length;
					const porcentajeFest =
						planilla.configuracion_salarial.porcentaje_festivos || 75;
					const salBasico =
						planilla.configuracion_salarial.salario_basico ||
						valorHoraBase *
							(planilla.configuracion_salarial.horas_mensuales_base || 240);
					const valDiario = salBasico / 30;
					const valDiarioConRecargo = valDiario * (porcentajeFest / 100);

					recargoRows.push([
						{
							text: [
								{
									text: 'BONO DÍAS FESTIVOS/DOMINICALES',
									fontSize: 9
								},
								{ text: '', fontSize: 9, color: '#007AFF' }
							],
							margin: [2, 2, 0, 2]
						},
						{
						...
						{
							text: formatCurrency(valDiarioConRecargo),
							fontSize: 9,
							bold: true,
							color,
							alignment: 'center' as const,
						...
						{
							text: diasFestivos.toString(),
							fontSize: 9,
							alignment: 'center' as const,
							margin: [0, 2, 0, 2]
						},
						{
							text: formatCurrency(planilla.total_festivos),
							fontSize: 9,
							bold: true,
							alignment: 'center' as const,
							margin: [0, 2, 0, 2]
