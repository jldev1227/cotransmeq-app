<script lang="ts">
	...
	async function cargarParaEditar(id: string) {
		...
		if (pernItem) {
			ext.pernote_unit = pernItem.valor_pernocte_unitario || 0;
			ext.pernote_cant = pernItem.cantidad_pernoctes || 0;
		}
	...
	$: terceroPlacasGroup = (() => {
		...
		terceroRows.forEach((t, i) => {
			...
			if (map.has(key)) {
				...
				map.set(key, {
					placa: t.placa,
					nombre: t.nombre_tercero || getTerceroNombre(t.placa, i) || '',
					identificacion: t.tercero_identificacion || '',
					tipo: t.tercero_tipo_persona || '',
					items: 1,
	...
	function handleRecargosPaste(e: ClipboardEvent) {
		...
		for (const line of lines) {
			...
			for (let i = 0; i < 31; i++)
				row.days[i] = parseFloat((cols[4 + i]?.trim() || '').replace(',', '.')) || 0;
			const off = 35;
			...
			const payload = {
				...
				items: rows.map((r, idx) => {
					...
					return {
						...
						tipo_servicio: 'HORA_24' as const,
	...
	<div class="page-wrap">
		<div class="topbar">
			<div class="topbar-l">
				...
				/>
				<div>
					<div class="t-title">⏳ Cargando Liquidación — OP-FR-07</div>
					<div class="t-sub">Gestión y vista previa de liquidaciones de servicios</div>
				</div>
			</div>
			<div style="display:flex;gap:10px">
				<button class="btn-back" on:click={() => goto(BACK_URL)}>← Volver</button>
			</div>
		</div>
		<div class="loading-center">
			<div class="spinner"></div>
			<span style="margin-top:12px;color:#64748b;font-size:13px">Cargando liquidación…</span>
		</div>
	</div>
{:else if loadError}
	<div class="page-wrap">
		<div class="topbar">
			<div class="topbar-l">
				...
				/>
				<div>
					<div class="t-title">⚠️ Error al cargar — OP-FR-07</div>
					<div class="t-sub">Gestión y vista previa de liquidaciones de servicios</div>
				</div>
			</div>
			<div style="display:flex;gap:10px">
		...
		<div class="error-fallback">
			...
			<div
				style="font-size:14px;color:#dc2626;font-weight:600;margin-bottom:8px;padding:10px 20px;background:#fef2f2;border:1px solid #fca5a5;border-radius:10px"
			>
				{loadError}
			</div>
			<div style="display:flex;gap:12px;margin-top:24px">
				<button class="btn-registrar" style="width:auto;margin:0" on:click={retryLoad}
					>🔄 Reintentar</button
				><button class="btn-back" on:click={() => goto(BACK_URL)}>← Volver</button>
			</div>
		</div>
	...
	<div class="page-wrap">
		...
		<div class="topbar">
			<div class="topbar-l">
				...
				<div>
					<div class="t-title">
						{editingId ? '✏️ Editar Liquidación' : '📋 Nueva Liquidación'} — OP-FR-07
					</div>
					<div class="t-sub">Gestión y vista previa de liquidaciones de servicios</div>
				</div>
			</div>
		...
		<div class="draft-pill">
			💾 Autoguardado habilitado{draftSavedAt
				? ` · último: ${draftSavedAt}`
				: ''}{#if draftAutoRestored}<span class="draft-restored">♻️ Cambios restaurados</span>{/if}
		</div>
		...
		<div class="card">
			...
			<div class="g3" style="margin-bottom:13px">
				<div style="grid-column:span 2">
					<label>Empresa Emisora</label><input bind:value={hdr.empresa} />
				</div>
				<div>
			...
			<div class="g4">
				<div>
					<label>Mes</label><select bind:value={hdr.mes}
						>{#each MESES as m}<option>{m}</option>{/each}</select
					>
				</div>
				<div>
					<label>Año</label><select bind:value={hdr.anio}
						>{#each YEARS as y}<option value={y}>{y}</option>{/each}</select
					>
				</div>
				<div>
					...
					<div class="ss-wrap">
						{#if selectedCliente}
							<div class="ss-selected">
								<span>{selectedCliente.nombre}</span><span class="ss-nit"
									>NIT: {selectedCliente.nit}</span
						...
						{:else}
							<input
								...
								on:keydown={(e) => {
									...
									if (e.key === 'ArrowDown') {
										e.preventDefault();
										clienteHighlight = Math.min(clienteHighlight + 1, clientesFiltrados.length - 1);
							...
							{#if clienteDropdown && clienteSearch.length > 0}
								<div class="ss-dropdown">
									{#if clientesFiltrados.length === 0}<div class="ss-empty">
											Sin resultados
										</div>{:else}{#each clientesFiltrados as c, ci}<div
												class="ss-option"
												...
												tabindex="-1"
											>
												<span class="ss-opt-name">{c.nombre}</span><span class="ss-opt-nit"
													>NIT: {c.nit}</span
												>
											</div>{/each}{/if}
								</div>
				...
				<div>
					<label>NIT (auto)</label><input
						value={selectedCliente ? selectedCliente.nit : ''}
						disabled
						style="background:#f1f5f9;font-weight:700;color:{selectedCliente
							? '#9a3412'
							: '#94a3b8'}"
					/>
				</div>
			...
			<div class="g2" style="margin-top:13px">
				<div>
					<label>Operadora</label><select bind:value={hdr.operadora}
						><option>PAREX</option><option>GEOPARK</option><option>OTRA</option></select
					>
				</div>
				<div>
					<label>Orden de Servicio (OSI)</label><input
						bind:value={hdr.osi}
						placeholder="OSI-####"
			...
			<div style="margin-top:10px">
				<label>Observaciones del Documento</label><input
					bind:value={hdr.observaciones}
					placeholder="Observaciones para el documento..."
				/>
			</div>
		</div>
		...
		<div class="card">
			...
			<div class="tbl-s">
				<table class="tbl">
					<thead
						><tr
							><th style="width:80px">Placa</th><th style="width:108px">F. Inicial</th><th
								style="width:108px">F. Final</th
							><th>Recorrido</th><th style="width:195px">Tipo</th><th style="width:58px">Cant.</th
							><th style="width:112px">Vr. Unitario</th><th style="width:110px">Subtotal</th><th
								style="width:64px">Dcto%</th
							><th style="width:112px">Vr. Final</th><th style="width:86px">Planilla</th><th
								style="width:34px"
							></th></tr
						></thead
					>
						...
						{#each rows as row (row.id)}
							...
							<tr>
								<td>
									<div class="ss-wrap">
										{#if row.placa}
											<div class="ss-selected" style="padding:4px 8px;font-size:11px">
												<span style="font-family:monospace;font-weight:800">{row.placa}</span
												><button
													class="ss-clear"
													style="font-size:12px"
													on:click={() => clearPlaca(row.id)}>✕</button
												>
											</div>
										{:else}
											<input
												style="font-family:monospace;font-weight:700;font-size:11px;text-transform:uppercase;padding:5px 6px"
												placeholder="placa..."
											...
											{#if row.placa_dropdown && row.placa_search.length > 0}
												<div class="ss-dropdown" style="min-width:160px">
													{#each placasFiltradas(row.placa_search) as v, vi}<div
															class="ss-option"
															class:highlighted={vi === row.placa_highlight}
															on:click={() => selectPlaca(row.id, v)}
															on:mouseenter={() => {
																row.placa_highlight = vi;
															}}
															role="option"
															aria-selected={vi === row.placa_highlight}
															tabindex="-1"
														>
															<span class="ss-opt-placa">{v.placa}</span><span class="ss-opt-info"
																>{v.tipo || ''} {v.marca || ''}</span
															>
														</div>{:else}<div class="ss-empty">Sin resultados</div>{/each}
												</div>
											{/if}
										{/if}
									</div>
								...
								>
								<td
									><select style="font-size:11px;padding:5px 6px" bind:value={row.tipo}
										>{#each TIPOS as t}<option>{t}</option>{/each}</select
									></td
								>
								<td
									><input
								...
								<td
									><input
										type="text"
										inputmode="numeric"
										style="padding:5px 6px;font-size:12px;text-align:right;font-family:monospace"
								...
								>
								<td
									>{#if rows.length > 1}<button class="delbtn" on:click={() => delRow(row.id)}
											>✕</button
										>{/if}</td
								>
							</tr>
		...
		<div class="card">
			...
			<div class="recargos-scroll" on:paste={handleRecargosPaste}>
				<table class="rtbl">
					<thead
						><tr
							><th class="sticky-col rcol-act">✕</th><th class="sticky-col rcol-emp">Empresa</th><th
								class="rcol-plan">Planilla</th
							><th class="rcol-plac">Placa</th><th class="rcol-cond">Conductor</th
							>{#each Array(31) as _, i}<th class="rcol-day">{i + 1}</th>{/each}<th class="rcol-tot"
								>Total</th
							><th class="rcol-tot">Prom</th><th class="rcol-rec">HED</th><th class="rcol-rec"
								>HEN</th
							><th class="rcol-rec">HEFD</th><th class="rcol-rec">HEFN</th><th class="rcol-rec"
								>RNDF</th
							><th class="rcol-rec">RN</th><th class="rcol-rec">RD</th></tr
						></thead
					>
					<tbody>
						{#each recargosRows as row (row.id)}
							<tr>
								<td class="sticky-col rcol-act"
									><button class="delbtn" on:click={() => delRecargoRow(row.id)}>✕</button></td
								>
								<td class="sticky-col rcol-emp"
									><input
										type="text"
										value={row.empresa}
										on:input={(e) => updRecargoField(row.id, 'empresa', e.currentTarget.value)}
									/></td
								>
								<td class="rcol-plan"
									><input
										type="text"
										value={row.planilla}
										on:input={(e) => updRecargoField(row.id, 'planilla', e.currentTarget.value)}
									/></td
								>
								<td class="rcol-plac"
									><input
										type="text"
										value={row.placa}
										on:input={(e) => updRecargoField(row.id, 'placa', e.currentTarget.value)}
									/></td
								>
								<td class="rcol-cond"
									><input
										type="text"
										value={row.conductor}
										on:input={(e) => updRecargoField(row.id, 'conductor', e.currentTarget.value)}
									/></td
								>
								{#each Array(31) as _, i}<td class="rcol-day"
										><input
											type="number"
											step="0.5"
											value={row.days[i] || ''}
											on:input={(e) => updRecargoDay(row.id, i, e.currentTarget.value)}
										/></td
									>{/each}
								<td class="rcol-tot mono">{row.total.toFixed(1)}</td><td class="rcol-tot mono"
									>{row.promedio.toFixed(1)}</td
								>
								<td class="rcol-rec"
									><input
										type="number"
										step="0.01"
										value={row.hed || ''}
										on:input={(e) => updRecargoField(row.id, 'hed', e.currentTarget.value)}
									/></td
								>
								<td class="rcol-rec"
									><input
										...
										step="0.01"
										value={row.hen || ''}
										on:input={(e) => updRecargoField(row.id, 'hen', e.currentTarget.value)}
									/></td
								>
								<td class="rcol-rec"
									><input
										...
										step="0.01"
										value={row.hefd || ''}
										on:input={(e) => updRecargoField(row.id, 'hefd', e.currentTarget.value)}
									/></td
								>
								<td class="rcol-rec"
									><input
										...
										step="0.01"
										value={row.hefn || ''}
										on:input={(e) => updRecargoField(row.id, 'hefn', e.currentTarget.value)}
									/></td
								>
								<td class="rcol-rec"
									><input
										...
										step="0.01"
										value={row.rndf || ''}
										on:input={(e) => updRecargoField(row.id, 'rndf', e.currentTarget.value)}
									/></td
								>
								<td class="rcol-rec"
									><input
										...
										step="0.01"
										value={row.rn || ''}
										on:input={(e) => updRecargoField(row.id, 'rn', e.currentTarget.value)}
									/></td
								>
								<td class="rcol-rec"
									><input
										...
										step="0.01"
										value={row.rd || ''}
										on:input={(e) => updRecargoField(row.id, 'rd', e.currentTarget.value)}
									/></td
								>
							</tr>
						{/each}
						{#if recargosRows.length === 0}<tr
								><td colspan={5 + 31 + 9} class="empty-rec"
									>Sin recargos. Pega datos o agrega filas manualmente.</td
								></tr
							>{/if}
						{#if recargosRows.length > 0}
							<tr class="totals-row"
								><td class="sticky-col"></td><td class="sticky-col"><b>TOTALES</b></td><td></td><td
								></td><td></td>{#each Array(31) as _, i}<td class="rcol-day mono"
										>{recargosTotals.days[i].toFixed(1)}</td
									>{/each}<td class="rcol-tot mono"><b>{recargosTotals.total.toFixed(1)}</b></td><td
								></td><td class="mono">{recargosTotals.hed.toFixed(2)}</td><td class="mono"
									>{recargosTotals.hen.toFixed(2)}</td
								><td class="mono">{recargosTotals.hefd.toFixed(2)}</td><td class="mono"
									>{recargosTotals.hefn.toFixed(2)}</td
								><td class="mono">{recargosTotals.rndf.toFixed(2)}</td><td class="mono"
									>{recargosTotals.rn.toFixed(2)}</td
								><td class="mono">{recargosTotals.rd.toFixed(2)}</td></tr
							>
						{/if}
					</tbody>
				</table>
		...
		{#if recargosRows.length > 0}
			<div class="card">
				...
				<div class="liq-cfg-grid">
					<div class="liq-cfg-item">
						<label>Salario Básico ($)</label><input
							type="text"
							inputmode="numeric"
					...
					<div class="liq-cfg-item">
						<label>Cargo</label><input type="text" bind:value={liqCfg.cargo} />
					</div>
					<div class="liq-cfg-item">
						<label>Valor Hora ($){liqCfg.valor_hora_override > 0 ? ' ✏️' : ' (auto)'}</label><input
							type="text"
							inputmode="decimal"
							value={fmtCOPDec(
					...
					<div class="liq-cfg-item">
						<label>Conductor Adicional ($)</label><input
							type="text"
							inputmode="numeric"
					...
					<div class="liq-cfg-item">
						<label>Seg. Social %</label><input
							type="number"
							step="0.01"
							bind:value={liqCfg.pct_seg_social}
						/>
					</div>
					<div class="liq-cfg-item">
						<label>Prestaciones %</label><input
							type="number"
							step="0.01"
							bind:value={liqCfg.pct_prestaciones}
						/>
					</div>
					<div class="liq-cfg-item">
						<label>Administración %</label><input
							type="number"
							step="0.01"
							bind:value={liqCfg.pct_admin}
						/>
					</div>
					<div class="liq-cfg-item">
						<label>Prueba COVID ($)</label><input
							type="text"
							inputmode="numeric"
				...
				<div class="liq-summary">
					<span>Subtotal 1: <b>{fmtDec1(liqSubtotal1)}</b></span><span
						>Seg. Social: <b>{fmtDec1(liqSegSocial)}</b></span
					>
					<span>Prestaciones: <b>{fmtDec1(liqPrestaciones)}</b></span><span
						>Admin: <b>{fmtDec1(liqAdmin)}</b></span
					>
		...
		<div class="tw">
			<div class="card">
				...
				<div style="display:flex;flex-direction:column;gap:12px">
					<div class="g2">
						<div>
							<label>Pernocte — Valor Unitario</label><input
								type="text"
								inputmode="numeric"
						...
						<div>
							<label>Pernocte — Cantidad</label><input
								type="number"
								bind:value={ext.pernote_cant}
								min="0"
							/>
						</div>
					</div>
					<div style="width:140px">
						<label>IVA %</label><input type="number" bind:value={ext.iva_pct} min="0" max="100" />
					</div>
				</div>
			</div>
			<div class="card">
				...
				<div style="padding:2px 0">
					<div class="sl">
						<span>Valor Total Servicios sin Recargos</span><strong style="color:#9a3412"
							>{COP(totalSvc)}</strong
						>
					</div>
					<div class="sl">
						<span>Valor Total Recargos</span><strong style="color:#2563eb">{COP(valRec)}</strong>
					</div>
					<div class="sl">
						<span>Pernote ({ext.pernote_cant} noche{ext.pernote_cant !== 1 ? 's' : ''})</span
						><strong style="color:#7c3aed">{COP(valPern)}</strong>
					</div>
					<div class="sl" style="border-top:2px solid #e2e8f0;margin-top:6px;padding-top:10px">
						<span>Subtotal</span><strong>{COP(subtotal)}</strong>
					</div>
					<div class="sl">
						<span style="color:#64748b">IVA {ext.iva_pct}%</span><span>{COP(ivaVal)}</span>
					</div>
		...
		{#if canSeeTerceros && rows.length > 0}
			<div class="card" style="margin-top:18px">
				...
				>
					<span>👥 Liquidación de Terceros</span><button
						class="btn-sync-terc"
						on:click={resetTerceroFromItems}>🔄 Sincronizar desde Servicios</button
					>
				</div>
				<div class="tbl-s">
					<table class="tbl terc-tbl">
						<thead
							><tr
								><th style="width:36px">#</th><th style="width:80px">Placa</th><th
									style="min-width:170px">Nombre 3°</th
								><th style="min-width:140px">Descripción</th><th style="width:140px">Fechas</th><th
									style="width:112px">V/Unidad</th
								><th style="width:64px">Cant.</th><th style="width:112px">Total Fact.</th><th
									style="width:64px">Admin%</th
								><th style="width:112px">Admon$</th><th style="width:112px">V/Liquidar</th><th
									style="width:112px">Ing.Extra Glob.</th
								><th style="width:112px">Ing.Extra Aval</th><th style="width:112px"
									>Ing. Cotransmeq</th
						...
						<tbody>
							{#each terceroRows as t, i}
								...
								<tr>
									...
									<td style="font-family:monospace;font-weight:800;font-size:11px;color:#9a3412"
										>{t.placa || '—'}</td
									>
									<td>
										<div class="ss-wrap terc-ss">
											{#if t.tercero_id && t.nombre_tercero}
												<div class="ss-selected" style="padding:4px 8px;font-size:11px">
													...
													><button
														class="ss-clear"
														on:click={() => clearTercero(i)}
														style="font-size:11px;padding:0 3px">✕</button
													>
												</div>
											{:else}
												<input
												...
												{#if terceroSearchIdx === i && (terceroResults.length > 0 || terceroSearching || terceroSearchText.length > 0)}
													<div class="ss-dropdown" style="font-size:11px">
														{#if terceroSearching}<div class="ss-empty">
																Buscando...
															</div>{:else if terceroResults.length === 0 && terceroSearchText.length > 0}<div
																class="ss-empty"
															>
																Sin resultados
															</div>{:else}{#each terceroResults as tc, ci}<div
																	class="ss-option"
																	...
																	tabindex="-1"
																>
																	<span class="ss-opt-name" style="font-size:11px"
																		>{tc.nombre_completo}</span
																	><span class="ss-opt-nit" style="font-size:10px"
									...
									<td
										><input
											...
											placeholder="ej. 13 FEB o 13 FEB - 14 FEB"
											style="font-size:11px;padding:5px 6px;text-transform:uppercase"
										/></td
									>
									<td
										><input
											...
											style="font-size:12px;padding:5px 6px;text-align:right;font-family:monospace"
										/></td
									>
									<td
										><input
											type="number"
											bind:value={terceroRows[i].cant}
											min="0"
											step="0.5"
											style="font-size:12px;padding:5px 5px;text-align:center"
										/></td
									>
									...
									<td class="calc" style="color:#059669;font-weight:700"
										>{COP(calc?.extraGlobal || 0)}</td
									>
									<td
										><input
											type="text"
										...
										/></td
									>
									<td class="calc" style="color:#2563eb;font-weight:700"
										>{COP(calc?.ingresoTransmeralda || 0)}</td
									>
									<td style="text-align:center"
										><button class="btn-del-terc" on:click={() => delTerceroRow(i)}>🗑</button></td
									>
								</tr>
							{/each}
						</tbody>
						<tfoot
							><tr class="terc-totals-row"
								><td
									colspan="8"
									style="text-align:right;font-weight:800;text-transform:uppercase;font-size:10px;letter-spacing:.05em"
									>TOTALES</td
								><td class="calc-g">{COP(tercTotalFacturado)}</td><td></td><td
									class="calc"
									style="color:#dc2626;font-weight:700">{COP(tercTotalAdmon)}</td
								><td class="calc-g">{COP(tercTotalVLiquidar)}</td><td
									class="calc"
									style="color:#059669;font-weight:700">{COP(tercTotalExtraGlobal)}</td
								><td class="calc" style="font-weight:700">{COP(tercTotalExtraAval)}</td><td
									class="calc"
									style="color:#2563eb;font-weight:700">{COP(tercTotalIngresoTrans)}</td
								><td></td></tr
							></tfoot
						>
					</table>
				...
				<div
					style="margin-top:16px;padding:14px;background:#f8fafc;border-radius:10px;border:1px solid #e2e8f0"
				>
					<h4 style="margin:0 0 10px;font-size:14px;color:#334155">📋 Resumen Terceros</h4>
					<div class="sl">
						<span>Ingreso Total Tercero (V/Liquidar)</span><strong style="color:#9a3412"
							>{COP(tercIngresoTotalTercero)}</strong
						>
					</div>
					<div class="sl">
						<span>Administración Cotransmeq</span><strong style="color:#dc2626"
							>{COP(tercAdminCotransmeq)}</strong
						>
					</div>
					<div class="sl">
						<span>Ingreso Extra Cotransmeq</span><strong style="color:#2563eb"
							>{COP(tercIngresoExtraTrans)}</strong
						>
					</div>
					<div class="sl" style="border-top:2px solid #e2e8f0;margin-top:6px;padding-top:8px">
						<span style="font-weight:700">INGRESO TOTAL COTRANSMEQ</span><strong
							style="color:#7c3aed">{COP(tercIngresoTotalCotransmeq)}</strong
						>
					</div>
					<div class="stotal">
						<span>VALOR TOTAL A FACTURAR</span><span>{COP(tercValorTotalFacturar)}</span>
					</div>
				</div>
			</div>
		{/if}
		...
		<div style="display:flex;justify-content:flex-end;gap:12px;margin-top:20px">
			<button
				...
				on:click={registrarLiquidacion}
				disabled={saving}
				>{#if saving}⏳ {editingId ? 'Actualizando...' : 'Registrando...'}{:else}{editingId
						? '💾 Actualizar Liquidación'
						: '📝 Registrar Liquidación'}{/if}</button
			>
			<button
				class="btn-registrar"
				style="background:#64748b;margin-top:0;width:auto"
	...
	<div class="pdf-wrap">
			...
			<div style="display:flex;gap:10px;align-items:center">
				<div class="page-tabs">
					<button
						...
						on:click={() => (previewPage = 'liquidacion')}>Hoja 1 · Liquidación</button
					>
					<button
						class="ptab"
						class:active={previewPage === 'recargos'}
						on:click={() => (previewPage = 'recargos')}>Hoja 2 · Recargos</button
					>
					<button
						class="ptab"
						class:active={previewPage === 'liquidador'}
						on:click={() => (previewPage = 'liquidador')}>Hoja 3 · Liquidador</button
					>
					{#if canSeeTerceros}<button
							class="ptab"
							class:active={previewPage === 'terceros'}
							on:click={() => (previewPage = 'terceros')}>Hoja 4 · Terceros</button
						>{/if}
				</div>
				<div class="zoom-controls">
					<button class="zoom-btn" on:click={() => (pdfZoom = Math.max(0.5, pdfZoom - 0.1))}
						>−</button
					><span class="zoom-label">{Math.round(pdfZoom * 100)}%</span><button
						class="zoom-btn"
						on:click={() => (pdfZoom = Math.min(2, pdfZoom + 0.1))}>+</button
					><button class="zoom-btn zoom-reset" on:click={() => (pdfZoom = 1)}>↺</button>
				</div>
				<button
					class="pbtn pbtn-back"
					on:click={() => {
						if (viewMode && editId) goto(BACK_URL);
						else setView('editor');
					}}>{viewMode ? '← Volver' : '← Editar'}</button
				>
		...
		{#if editingId && liqEstado !== 'BORRADOR'}
			...
			<div class="estado-bar">
				<span class="estado-badge" style="background:{badge.bg};color:{badge.text}"
					>{badge.label}</span
				>
				{#if liqFechaLiquidacion && ['LIQUIDADA', 'APROBADA', 'FACTURADA'].includes(liqEstado)}<span
						class="estado-info"
						>📋 Liquidada: <strong>{fmtFecha(liqFechaLiquidacion)}</strong>{#if liqLiquidadoPor}
							por {liqLiquidadoPor.nombre}{/if}</span
					>{/if}
				{#if liqFechaAprobacion && ['APROBADA', 'FACTURADA'].includes(liqEstado)}<span
						class="estado-info"
						>✅ Aprobada: <strong>{fmtFecha(liqFechaAprobacion)}</strong>{#if liqAprobadoPor}
							por {liqAprobadoPor.nombre}{/if}</span
					>{/if}
			...
			<div class="pdf-body print-sheet">
				<div class="page" style="transform: scale({pdfZoom}); transform-origin: top center;">
					...
					<table class="st">
						...
						>
						<tbody
							>{#each rows as row (row.id)}{@const { sub, vf } = calcRow(row)}<tr
									...
									><td style="font-size:7pt">{row.tipo}</td><td class="tc" style="font-weight:700"
										>{row.cant}</td
									><td class="mc">{COP(row.vr_unit)}</td><td class="mc">{COP(sub)}</td><td
										class="tc">{row.dcto}%</td
									><td class="mch">{COP(vf)}</td><td
										class="tc"
										style="font-family:monospace;font-size:7.2pt">{row.planilla}</td
									></tr
								>{/each}{#each Array(Math.max(0, 4 - rows.length)) as _}<tr class="filler"
						...
						<tfoot
							><tr
								><td
									colspan="9"
									style="text-align:right;color:#9a3412;padding-right:8px;font-size:8pt;font-weight:800"
									>TOTAL SERVICIOS:</td
					...
					<div class="bg">
						<div class="bl">
							...
							<div class="pernote-box">
								<div
									style="font-size:6.8pt;font-weight:800;color:#9a3412;padding:2px 6px;background:#ffedd5;border:1px solid #fed7aa;border-bottom:none"
								>
									PERNOCTE
								</div>
								<table class="pernote-tbl">
									<thead><tr><th>Vr. Unitario</th><th>Cantidad</th><th>Total</th></tr></thead><tbody
										><tr
											><td style="font-family:monospace;font-size:7.5pt">{COP(ext.pernote_unit)}</td
											><td style="font-weight:700">{ext.pernote_cant}</td><td
												style="font-weight:800;color:#9a3412">{COP(valPern)}</td
											></tr
										></tbody
									>
								</table>
			...
			<div class="pdf-body pdf-body-landscape print-sheet">
				...
				>
							...
							<tbody
								>{#each recargosRows as row (row.id)}<tr
										...
										>{#each Array(31) as _, i}<td
												class="rgt-d tc"
												style={row.days[i] > 0 ? 'font-weight:700;color:#9a3412' : 'color:#ccc'}
												>{row.days[i] ? row.days[i].toFixed(1) : '-'}</td
											>{/each}<td class="rgt-t tc" style="font-weight:800;color:#9a3412"
								...
								<tr class="rgt-totals"
									...
									>{#each Array(31) as _, i}<td class="rgt-d tc" style="font-weight:800"
										...
										>{/each}<td
										class="rgt-t tc"
										style="font-weight:900;color:#9a3412;font-size:7.5pt"
					...
					{#if recargosRows.length > 0}
						<div style="margin-top:10px">
							<div
								style="font-size:7.5pt;font-weight:800;color:#9a3412;margin-bottom:4px;text-transform:uppercase;letter-spacing:.05em"
							>
					...
					<div class="doc-ft">
						<span>OP-FR-06 · Versión 1</span><span
							>Generado el {new Date().toLocaleDateString('es-CO', {
								day: '2-digit',
								month: 'long',
								year: 'numeric'
						...
						<tbody>
							...
							<tr class="liq-total-row"
								><td
									...
									>TOTAL</td
								><td class="mc" style="font-weight:900;font-size:9.5pt;color:#9a3412"
									>{fmtDec1(liqTotal)}</td
								></tr
							>
						</tbody>
					</table>
					<div class="sigs" style="margin-top:20px">
						<div class="sig">
							<div class="sig-lbl">FIRMA AUTORIZADA POR CLIENTE:</div>
							<div class="sig-line">{selectedCliente?.nombre || ''}</div>
						</div>
						...
						<tbody
							>{#each terceroRows as t, i}{@const calc = terceroCalcs[i]}<tr
									...
									><td class="tc">{calc?.pctAdmin || 0}%</td><td class="mc" style="color:#b91c1c"
										>{COP(calc?.admon || 0)}</td
									><td class="mc" style="font-weight:700;color:#9a3412"
						...
						<tfoot
							><tr style="font-weight:800;background:#e2e8f0"
								...
								><td></td><td class="mc" style="color:#b91c1c">{COP(tercTotalAdmon)}</td><td
									class="mc"
									style="color:#9a3412">{COP(tercTotalVLiquidar)}</td
								></tr
							></tfoot
						>
					</table>
					<table class="terc-prev-tbl" style="margin-top:10px">
						...
						<tbody
							>{#each terceroPlacasGroup as g, gi}<tr
									><td class="tc">{gi + 1}</td><td
										class="tc"
										style="font-weight:700;font-family:monospace">{fmtPlaca(g.placa)}</td
									...
									><td class="tc">{g.items}</td><td class="mc" style="font-weight:700;color:#9a3412"
										>{COP(g.vLiquidar)}</td
									></tr
								>{/each}</tbody
						>
						<tfoot
							><tr style="font-weight:800;background:#e2e8f0"
								...
								><td class="tc" style="font-weight:700">{terceroRows.length}</td><td
									class="mc"
									style="color:#9a3412">{COP(tercTotalVLiquidar)}</td
								></tr
							></tfoot
							...
							<tbody>
								<tr
									><td class="ts-lbl">INGRESO TOTAL TERCERO (V/Liquidar)</td><td
										class="ts-val"
										style="color:#9a3412">{COP(tercIngresoTotalTercero)}</td
									></tr
								>
								<tr class="ts-sep"
								...
								>
								<tr
									><td class="ts-lbl">INGRESO EXTRA COTRANSMEQ</td><td
										class="ts-val"
										style="color:#2563eb">{COP(tercIngresoExtraTrans)}</td
									></tr
								>
								<tr class="ts-total"
									><td class="ts-lbl">INGRESO TOTAL COTRANSMEQ</td><td
										class="ts-val"
										style="color:#7c3aed">{COP(tercIngresoTotalCotransmeq)}</td
									></tr
								>
								<tr class="ts-grand"
	...
	<div class="modal-bg" on:click|self={() => (historialModalOpen = false)}>
		<div class="modal-box" style="max-width:580px">
			...
			<div class="modal-body">
				...
				{:else if historialData.length === 0}<div
						style="text-align:center;padding:40px 0;color:#94a3b8"
					>
						<span style="font-size:36px">📭</span>
						<p style="margin:10px 0 0;font-size:14px">Sin registros</p>
					</div>
				{:else}
					<div class="historial-timeline">
						{#each historialData as h, i}
							...
							<div class="historial-entry" class:historial-entry-first={i === 0}>
								<div
									class="historial-dot"
									style="background:{badge.bg};border-color:{badge.bg}"
								></div>
								<div class="historial-card">
									...
									>
										<div style="display:flex;align-items:center;gap:6px">
											{#if h.estado_anterior}{@const prev = getEstadoBadge(h.estado_anterior)}<span
													class="historial-tag"
													style="background:{prev.bg};color:{prev.text}">{prev.label}</span
												><span style="color:#94a3b8;font-size:14px;font-weight:700">→</span>{/if}
											<span class="historial-tag" style="background:{badge.bg};color:{badge.text}"
												>{badge.label}</span
											>
										</div>
										<span style="font-size:11.5px;color:#94a3b8;white-space:nowrap"
											>{fmtFecha(h.created_at)}</span
										>
									</div>
									{#if h.usuario}<div style="font-size:12.5px;color:#475569;margin-top:6px">
											👤 {h.usuario.nombre || h.usuario.correo}
										</div>{/if}
									{#if h.motivo}<div
											style="font-size:12px;color:#64748b;margin-top:4px;font-style:italic"
										>
											💬 {h.motivo}
										</div>{/if}
								</div>
...
<style>
	...
	.topbar {
		...
		box-shadow: 0 10px 40px rgba(154, 52, 18, 0.35);
	}
	.topbar-l {
		display: flex;
		align-items: center;
	...
	.btn-registrar:hover {
		...
		box-shadow: 0 8px 30px rgba(154, 52, 18, 0.45);
	}
	...
	.pbtn-print {
		...
		color: #fff;
		box-shadow: 0 2px 12px rgba(194, 65, 12, 0.4);
	}
	.pbtn-print:hover {
		transform: translateY(-1px);
	}
	...
	.success-overlay {
		...
		align-items: center;
		justify-content: center;
		background: linear-gradient(135deg, rgba(194, 65, 12, 0.95) 0%, rgba(234, 88, 12, 0.95) 100%);
	...
	.print-modal {
		...
		overflow: hidden;
		animation: pmSlide 0.25s cubic-bezier(0.175, 0.885, 0.32, 1.275);
	}
	@keyframes pmSlide {
		from {
			opacity: 0;
	...
	.print-modal-go {
		background: linear-gradient(135deg, #9a3412, #c2410c);
		color: #fff;
		box-shadow: 0 4px 16px rgba(154, 52, 18, 0.35);
	}
	.print-modal-go:hover {
		transform: translateY(-1px);
		box-shadow: 0 6px 24px rgba(154, 52, 18, 0.4);
	}
