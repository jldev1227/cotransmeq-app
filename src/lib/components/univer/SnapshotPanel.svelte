<!--
	...
	Drawer lateral compartido por los dos canvas anuales. El `scope` decide
...
<script lang="ts">
	...
	async function seleccionar(s: SnapshotResumen) {
		...
		if (scope !== 'adicionales') return;
		cargandoDiff = true;
		try {
			diff = await adicionalesSnapshotsAPI.diff(s.id);
		} catch (e: any) {
