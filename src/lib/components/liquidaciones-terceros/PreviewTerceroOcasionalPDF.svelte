<script lang="ts">
	...
	import type { PreviewDataOcasional } from '$lib/api/liquidaciones-terceros-ocasional';

	export let data: PreviewDataOcasional;
...
<div class="ocas-root pdf-wrap">
	...
	<div class="pdf-bar no-print">
		<div class="pdf-bar-l">
			...
			<div class="pdf-bar-text">
				<div class="pb-t font-display" style="font-family:'Fraunces',Georgia,serif;font-weight:500">
					Liquidación Ocasional de Adicionales · Terceros
				</div>
				<div class="pb-s">
		...
		<div class="pdf-bar-actions">
			...
			<!-- `history.back()` y no un `goto` fijo: el único acceso a esta vista es el
			     botón "Vista previa" del canvas ocasional, y un destino fijo perdería el
			     año/mes/hoja que el usuario tenía abiertos. Mismo criterio que
			     `ocasional/[id]/+page.svelte`. -->
			<button class="pbtn pbtn-back" onclick={() => history.back()}>
	...
	<div class="estado-bar no-print">
		<span
			class="status-pill"
			style="background:rgba(249, 115, 22,0.10);color:#c2410c;border:1px solid rgba(249, 115, 22,0.28);font-size:11px;padding:3px 10px"
		>
			{cabecera?.estado || '—'}
		</span>
		<div class="estado-info-stack">
			...
			<span class="estado-info">
				...
				<span class="estado-info-full">
					...
					<strong class="font-mono" style="color:#166534;font-weight:700;letter-spacing:0.02em">{COP(totales?.total_pagar || 0)}</strong>
				</span>
			</span>
		</div>
	</div>
...
<style>
	/*
	 * TODAS las reglas van confinadas bajo `.ocas-root`.
	 *
	 * Antes eran `:global(...)` a secas, y este componente redefine `.page`,
	 * `.mc`, `.terc-prev-tbl` y `.code-badge` con valores distintos a los de
	 * `PreviewTerceroPDF.svelte`. Como `:global` no se limita al componente,
	 * cualquier propiedad que este fichero declarase y el otro no se colaba
	 * en el otro preview con solo estar montado — y el síntoma era un
	 * documento que cambiaba de aspecto según qué se hubiera abierto antes.
	 *
	 * Siguen siendo `:global` (y no reglas normales) porque el marcado usa
	 * clases compartidas con el sistema; lo que las hace seguras es el
	 * ancestro, no el alcance.
	 */
	:global(.ocas-root.pdf-wrap) {
		position: relative;
		display: flex;
	...
	:global(.ocas-root .pdf-bar-l) {
		display: flex;
		align-items: center;
		gap: 12px;
	}
	:global(.ocas-root .pb-logo) {
		height: 38px;
		width: auto;
	}
	:global(.ocas-root .pdf-bar-text .pb-t) {
		font-size: 14px;
		color: #0f172a;
	}
	:global(.ocas-root .pdf-bar-text .pb-s) {
		font-size: 11px;
		color: #6b6b6b;
		margin-top: 2px;
	}
	:global(.ocas-root .code-badge) {
	...
	:global(.ocas-root .pdf-bar-actions) {
		display: flex;
		align-items: center;
		gap: 8px;
	}
	:global(.ocas-root .zoom-controls) {
		display: flex;
		align-items: center;
		gap: 2px;
		padding: 2px;
	...
	:global(.ocas-root .zoom-btn:hover) {
		background: rgba(0, 0, 0, 0.05);
	}
	:global(.ocas-root .zoom-label) {
		font-family: 'JetBrains Mono', monospace;
		font-size: 10px;
		color: #6b6b6b;
		padding: 0 6px;
	...
	:global(.ocas-root .pbtn-back) {
		background: white;
		border-color: rgba(0, 0, 0, 0.1);
		color: #1a1a1a;
	}
	:global(.ocas-root .pbtn-back:hover) {
		background: #fcfcfb;
	}
	:global(.ocas-root .pbtn-icon-svg) {
		height: 14px;
		width: 14px;
	}
	:global(.ocas-root .estado-bar) {
		display: flex;
		align-items: center;
		gap: 24px;
		padding: 12px 28px;
	...
	:global(.ocas-root .estado-info-stack) {
		display: flex;
		gap: 24px;
		flex: 1;
	}
	:global(.ocas-root .estado-info) {
		display: flex;
		align-items: center;
		gap: 8px;
	}
	:global(.ocas-root .estado-info-icon) {
		display: flex;
		width: 22px;
		height: 22px;
		align-items: center;
	...
	:global(.ocas-root .estado-info-icon svg) {
		width: 16px;
		height: 16px;
	}
	:global(.ocas-root .filter-field-label) {
		font-size: 0.55rem;
		font-weight: 600;
		text-transform: uppercase;
	...
	:global(.ocas-root .estado-info-full) {
		font-size: 12px;
		color: #1a1a1a;
	}
	:global(.ocas-root .font-mono) {
		font-family: 'JetBrains Mono', monospace;
	}
	:global(.ocas-root .pdf-body) {
		flex: 1;
		overflow: auto;
		padding: 28px;
		background: #fcfcfb;
	}
	:global(.ocas-root .page-scale-wrap) {
		margin: 0 auto;
	}
	:global(.ocas-root .page) {
		background: white;
		padding: 14mm;
		width: 1900px;
		min-height: 800px;
	...
	:global(.ocas-root .dh-title .dh-co) {
		font-size: 14pt;
		font-weight: 800;
		color: #0f4025;
		letter-spacing: 0.02em;
	}
	:global(.ocas-root .dh-title .dh-doc) {
		font-size: 9pt;
		font-weight: 600;
		color: #4a4a4a;
		margin-top: 2px;
	}
	:global(.ocas-root .dh-meta .mt) {
		font-family: 'JetBrains Mono', monospace;
		font-size: 8pt;
	}
	:global(.ocas-root .dh-meta .ml) {
		padding: 1px 6px;
		color: #6b6b6b;
		text-align: right;
	}
	:global(.ocas-root .dh-meta .mv) {
		padding: 1px 6px;
		color: #0f4025;
		font-weight: 600;
	}
	...
	:global(.ocas-root .pc .pcval) {
		font-size: 10pt;
		font-weight: 700;
		color: #0f172a;
	}
	:global(.ocas-root .pc-tercero .pcval) {
		font-size: 9pt;
		max-width: 320px;
		overflow: hidden;
	...
	:global(.ocas-root .terc-prev-tbl) {
		width: 100%;
		border-collapse: collapse;
		font-family: 'Inter', system-ui;
	}
	:global(.ocas-root .terc-prev-tbl th) {
		background: #0f4025;
		color: white;
		padding: 5px 6px;
		font-size: 9pt;
	...
	:global(.ocas-root .terc-prev-tbl td) {
		padding: 4px 6px;
		border-bottom: 1px solid rgba(0, 0, 0, 0.05);
		font-size: 9.5pt;
		color: #1a1a1a;
	}
	:global(.ocas-root .num-cell) {
		text-align: center;
	}
	:global(.ocas-root .mc) {
		text-align: right;
	...
	:global(.ocas-root .empty-desc) {
		text-align: center;
		color: #9a9a9a;
		font-style: italic;
		padding: 14px;
	}
	:global(.ocas-root .desc-section-title) {
		font-size: 10pt;
		font-weight: 800;
		text-transform: uppercase;
		letter-spacing: 0.06em;
	...
	:global(.ocas-root .resumen-line > span:first-child) {
		text-transform: uppercase;
		letter-spacing: 0.04em;
	}
	:global(.ocas-root .resumen-val) {
		font-family: 'JetBrains Mono', monospace;
	}
	:global(.ocas-root .resumen-red) {
		color: #dc2626;
	}
	:global(.ocas-root .resumen-pagar) {
		color: #0f4025;
		font-size: 14pt;
		border-top: 1.5px solid #0f4025;
		padding-top: 4px;
	}
	:global(.ocas-root .resumen-pagar .resumen-val) {
		font-size: 14pt;
	}
	:global(.ocas-root .sigs) {
		display: grid;
	...
	:global(.ocas-root .sig-img) {
		max-height: 60px;
		max-width: 280px;
		object-fit: contain;
		display: block;
	}
	:global(.ocas-root .sig-lbl) {
		font-size: 9pt;
		font-weight: 700;
		color: #0f4025;
		text-transform: uppercase;
