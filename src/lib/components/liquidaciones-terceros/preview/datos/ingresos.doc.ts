...
import { esAdicional } from '$lib/editor/business/ingresos-transmeralda';
import type {
	BloquePreview,
	ColumnaPreview,
	DocumentoPreview,
...
export const HOJAS_INGRESOS = {
	INGRESOS: {
		...
		/// Etiqueta del NOMBRE DE ARCHIVO: `INGRESOS_JUNIO_2026`. Es «INGRESOS»
		/// y no «otros_ingresos» porque el nombre se lee en una carpeta junto a
		/// los de los otros canvas, y ahí lo que distingue es el canvas del que
		/// salió, no el título largo del formato.
		archivo: 'INGRESOS'
	},
	ADICIONALES: {
...
function agruparPorEmpresa(filas: FilaCalculada[]): GrupoEmpresa[] {
	...
	for (const f of filas) {
		// LOS ADICIONALES SE AGRUPAN POR PLACA, no todos juntos.
		//
		// Consolidar por empresa vale para un cliente: sus servicios son suyos y
		// sumarlos dice algo. «COTRANSMEQ» no es un cliente —es la marca de que
		// la fila es un adicional que paga la empresa—, así que la clave a secas
		// metía en una sola línea once pagos de cinco vehículos distintos, con la
		// descripción del primero y un total que no se podía cotejar contra
		// ninguna hoja de placa. Con la placa en la clave, cada línea vuelve a ser
		// contrastable contra su cierre.
		const esAdic = esAdicional(f.item);
		const clave = esAdic
			? `adicional::${f.item.placa || '—'}`
			: f.item.cliente_id || f.item.cliente_nombre || '—';
		let g = grupos.get(clave);
		if (!g) {
			g = {
				empresa: esAdic
					? `${f.item.cliente_nombre || 'COTRANSMEQ'} · ${f.item.placa || '—'}`
					: f.item.cliente_nombre || '—',
...
export function documentoIngresos(o: {
	...
	return {
		...
		firmas: false,
		nombreArchivo: nombreArchivoHoja(identidad.archivo, o.mes, o.anio)
	};
}
